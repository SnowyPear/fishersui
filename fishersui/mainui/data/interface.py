...
def buildtree():
	...
	with open('received.csv', 'r' ) as file:
	  reader = csv.reader(file)
	  for row in reader:
	...
	with open('dispatched.csv', 'r' ) as file:
	  reader = csv.reader(file)
	  for row in reader:
	    a = datetime.datetime.strptime(row[1],'%d/%m/%Y').date()
		  ...
		  for o in owner:
		  	for oc in o.coat:
		  		if t in oc.id:
		  			...
		  			#print(o.name + ' - ' + oc.id)
		  			#print('dispatched')
		  			#if len(oc.dispatched) != 0:
		  			#	for d in oc.dispatched:
		  			#		print(d)
		  			#print('received')
		  			#if len(oc.received) != 0:
		  			#	for r in oc.received:
		  			#		print(r)
...
if __name__ == '__main__':
	...
	while True:
		parse(input('\n').strip(),owner)
		#print()
	#print([x for x in owner if x.name == "Lee Fernie"][0].id)